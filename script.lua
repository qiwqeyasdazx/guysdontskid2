-- Astral Hub V3 Library
-- Modified: Full Black UI + Gray Borders
-- Original source shared by araujozwx

if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("Astral Hub V3 GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name, "Astral Hub V3") then
				v:Destroy()
			end
		end
	end
...
local T1UIColor = {
    ...
    ["Click Effect Color"]              = Color3.fromRGB(130, 130, 130),
    ["Setting Icon Color"]              = Color3.fromRGB(160, 160, 160),
    ["Logo Image"]                      = "rbxassetid://92778989024364",
    ["Search Icon Color"]               = Color3.fromRGB(130, 130, 130),
    ...
    ["Placeholder Text Color"]          = Color3.fromRGB(70, 70, 70),
    ["Title Text Color"]                = Color3.fromRGB(255, 255, 255),

    -- FULL BLACK Backgrounds
    ["Background Main Color"]           = Color3.fromRGB(0, 0, 0),
    ["Background 1 Color"]              = Color3.fromRGB(5, 5, 5),
    ["Background 1 Transparency"]       = 0.0,
    ["Background 2 Color"]              = Color3.fromRGB(12, 12, 12),
    ["Background 3 Color"]              = Color3.fromRGB(8, 8, 8),
    ["Background Image"]                = "",

    -- Gray accents (replacing pink)
    ["Page Selected Color"]             = Color3.fromRGB(130, 130, 130),
    ["Section Text Color"]              = Color3.fromRGB(255, 255, 255),
    ["Section Underline Color"]         = Color3.fromRGB(100, 100, 100),
    ["Toggle Border Color"]             = Color3.fromRGB(80, 80, 80),
    ["Toggle Checked Color"]            = Color3.fromRGB(150, 150, 150),
    ["Toggle Desc Color"]               = Color3.fromRGB(155, 155, 155),

    ["Button Color"]                    = Color3.fromRGB(10, 10, 10),
    ["Label Color"]                     = Color3.fromRGB(8, 8, 8),
    ["Dropdown Icon Color"]             = Color3.fromRGB(130, 130, 130),
    ["Dropdown Selected Color"]         = Color3.fromRGB(130, 130, 130),
    ["Dropdown Selected Check Color"]   = Color3.fromRGB(200, 200, 200),

    ["Textbox Highlight Color"]         = Color3.fromRGB(130, 130, 130),
    ["Box Highlight Color"]             = Color3.fromRGB(130, 130, 130),
    ["Slider Line Color"]               = Color3.fromRGB(40, 40, 40),
    ["Slider Highlight Color"]          = Color3.fromRGB(160, 160, 160),
...
Library_Function.Gui.Name = 'Astral Hub V3 GUI'
...
screenGui.Name = "AstralToggleGui"
screenGui.ResetOnSpawn = false
screenGui.IgnoreGuiInset = true
screenGui.Parent = CoreGui

-- TOGGLE BUTTON - Full Black + Gray Border + New Icon
...
mainButton.Image = "rbxassetid://92778989024364"
...
UIS.InputEnded:Connect(function(input)
	...
	or input.UserInputType == Enum.UserInputType.Touch then
		...
		if math.abs(delta.X) < CLICK_DISTANCE and math.abs(delta.Y) < CLICK_DISTANCE then
			...
			if isToggled then
				TweenService:Create(mainButton, tweenOn,  { BackgroundColor3 = Color3.fromRGB(15, 15, 15) }):Play()
			else
				TweenService:Create(mainButton, tweenOff, { BackgroundColor3 = Color3.fromRGB(0, 0, 0) }):Play()
			end
		end
		dragging = false
	end
...
mainButton.MouseEnter:Connect(function()
	...
	TweenService:Create(UIStroke, fluentTweenInfo, { Transparency = 0.2 }):Play()
	TweenService:Create(mainButton, hoverTweenInfo,  { BackgroundColor3 = Color3.fromRGB(20, 20, 20) }):Play()
end)

mainButton.MouseLeave:Connect(function()
	local targetColor = isToggled and Color3.fromRGB(15, 15, 15) or Color3.fromRGB(0, 0, 0)
	TweenService:Create(mainButton, fluentTweenInfo, {
		Size = UDim2.new(0, 60, 0, 60),
		BackgroundTransparency = 0
	}):Play()
	TweenService:Create(UIStroke, fluentTweenInfo, { Transparency = 0.5 }):Play()
...
mainButton.MouseButton1Click:Connect(function()
	...
	local scaleTween    = TweenService:Create(mainButton, clickTweenInfo, { Size = UDim2.new(0, 55, 0, 55) })
	local scaleBackTween= TweenService:Create(mainButton, clickTweenInfo, { Size = UDim2.new(0, 60, 0, 60) })
	local iconScaleTween= TweenService:Create(icon,       clickTweenInfo, { Size = UDim2.new(0, 60, 0, 60) })
	local targetColor   = isToggled and Color3.fromRGB(15, 15, 15) or Color3.fromRGB(0, 0, 0)
...
Library.ToggleUI = function()
	...
	if game.CoreGui:FindFirstChild("Astral Hub V3 GUI") then
		for a, b in ipairs(game.CoreGui:GetChildren()) do
			if b.Name == "Astral Hub V3 GUI" then
				b.Enabled = getgenv().UIToggled
			end
		end
	end
	isToggled = getgenv().UIToggled
	local iconScaleTween = TweenService:Create(icon, defaultTweenInfo, { Size = UDim2.new(0, 60, 0, 60) })
	local targetColor    = isToggled and Color3.fromRGB(15, 15, 15) or Color3.fromRGB(0, 0, 0)
	local colorTween     = TweenService:Create(mainButton, defaultTweenInfo, { BackgroundColor3 = targetColor })
	iconScaleTween:Play()
	colorTween:Play()
...
NotiContainer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
...
local libCreateNoti = function(Setting)
	...
	-- gray border on notification
	local notiStroke = Instance.new("UIStroke", Noticontainer)
	notiStroke.Color = Color3.fromRGB(90, 90, 90)
	notiStroke.Thickness = 1

	Topnoti.Name = "Topnoti"
	Topnoti.Parent = Noticontainer
	Topnoti.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	Ruafimg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "200,200,200") .. ")\">" .. tostring(TitleNameMain or "Astral Hub V3") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
	TextLabelNoti.Name = "TextLabelNoti"
	TextLabelNoti.Parent = Topnoti
	TextLabelNoti.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	CloseContainer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	CloseImage.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	if Description then
		...
		TextLabelNoti2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
...
function Library:CreateWindow(Setting)
	local TitleNameMain = Setting.Title or "Astral Hub V3"
	...
	-- GRAY border stroke on main window
	local uistr = Instance.new("UIStroke", MainContainer)
	uistr.Thickness = 1
	uistr.Color = Color3.fromRGB(90, 90, 90)
	...
	Concacmain.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	TopStroke.BackgroundColor3 = Color3.fromRGB(90, 90, 90)
	TopStroke.BackgroundTransparency = 0.4
	...
	TextLabelMain.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	TextLabelMain.Text = "<font color=\"rgb(200,200,200)\">" .. tostring(TitleNameMain or "Astral Hub V3") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
	...
	PageControl.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
	PageControl.BackgroundTransparency = 0.05

	-- Gray border on page control panel
	local pageControlStroke = Instance.new("UIStroke", PageControl)
	pageControlStroke.Color = Color3.fromRGB(90, 90, 90)
	...
	ControlList.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	...
	PageSearch.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
	...
	SearchIcon.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
	...
	SearchBox.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
		...
		GlobalSearch = function(searchText)
			...
			for section, elements in pairs(elementsInSection) do
				...
				for _, elementInfo in ipairs(elements) do
					if elementInfo.sectionFound then shouldShowSection = true end
	...
	function Main_Function:AddTab(PageName, IconId)
		...
		if not IconId or IconId == "" then TabIcon.Visible = false end

		PageNameControl.Name = Page_Name .. "_Control"
		PageNameControl.Parent = ControlList
		PageNameControl.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		...
		TabTitleContainer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		...
		-- Gray border on page container
		local pageContainerStroke = Instance.new("UIStroke", PageContainer)
		pageContainerStroke.Color = Color3.fromRGB(80, 80, 80)
		pageContainerStroke.Thickness = 1
		...
		PageTitle.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		...
		PageList.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		...
		-- (Page-level search button — same logic as original, colors updated)
		local PageSearch2       = Instance.new("Frame")
		local PageSearch2Corner = Instance.new("UICorner")
		...
		PageSearch2.Name = "Page Search"
		PageSearch2.Parent = PageContainer
		PageSearch2.AnchorPoint = Vector2.new(1, 0)
		PageSearch2.BackgroundColor3 = getgenv().UIColor["Background 2 Color"]
		PageSearch2.Position = UDim2.new(1, -5, 0, 5)
		PageSearch2.Size = UDim2.new(0, 20, 0, 20)
		PageSearch2.ClipsDescendants = true
		PageSearch2Corner.CornerRadius = UDim.new(0, 2)
		PageSearch2Corner.Name = "PageSearchCorner"
		PageSearch2Corner.Parent = PageSearch2
		SearchFrame2.Name = "SearchFrame"
		SearchFrame2.Parent = PageSearch2
		SearchFrame2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		SearchFrame2.BackgroundTransparency = 1.000
		SearchFrame2.Size = UDim2.new(0, 20, 0, 20)
		SearchIcon2.Name = "SearchIcon"
		SearchIcon2.Parent = SearchFrame2
		SearchIcon2.AnchorPoint = Vector2.new(0.5, 0.5)
		SearchIcon2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		SearchIcon2.BackgroundTransparency = 1.000
		SearchIcon2.Position = UDim2.new(0.5, 0, 0.5, 0)
		SearchIcon2.Size = UDim2.new(0, 16, 0, 16)
		...
		SearchButton2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		SearchButton2.BackgroundTransparency = 1.000
		SearchButton2.Size = UDim2.new(1, 0, 1, 0)
		SearchButton2.Font = Enum.Font.SourceSans
		SearchButton2.Text = ""
		SearchButton2.TextColor3 = Color3.fromRGB(255, 255, 255)
		SearchButton2.TextSize = 14.000
		SearchBox2.Name = "Search Box"
		SearchBox2.Parent = PageSearch2
		SearchBox2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		SearchBox2.BackgroundTransparency = 1.000
		SearchBox2.Position = UDim2.new(0, 30, 0, 0)
		SearchBox2.Size = UDim2.new(1, -30, 1, 0)
		SearchBox2.Font = Enum.Font.GothamBold
		SearchBox2.Text = ""
		SearchBox2.TextSize = 14.000
		SearchBox2.TextXAlignment = Enum.TextXAlignment.Left
		SearchBox2.PlaceholderText = "Search Section name"
		SearchBox2.PlaceholderColor3 = getgenv().UIColor["Placeholder Text Color"]
		SearchBox2.TextColor3 = getgenv().UIColor["Text Color"]

		local Openned2 = false
		...
		SearchButton2.MouseButton1Click:Connect(function()
			Openned2 = not Openned2
			local size = Openned2 and UDim2.new(0, 175, 0, 20) or UDim2.new(0, 20, 0, 20)
			game.TweenService:Create(PageSearch2, TweenInfo.new(getgenv().UIColor["Tween Animation 2 Speed"]), { Size = size }):Play()
		end)

		local function hideOtherFrame()
			for i, v in next, PageList:GetChildren() do
				if not v:IsA('UIListLayout') then v.Visible = false end
			end
		end
		local function showFrameName()
			for i, v in pairs(PageList:GetChildren()) do
				if not v:IsA('UIListLayout') then
					if string.find(string.lower(v.Name), string.lower(SearchBox2.Text)) then v.Visible = true end
				end
			end
		end
		SearchBox2:GetPropertyChangedSignal("Text"):Connect(function()
			hideOtherFrame() showFrameName()
		end)

		for i, v in pairs(ControlList:GetChildren()) do
			if not (v:IsA('UIListLayout')) then
				if i == 2 then v.Frame.Line.PageInLine.BackgroundTransparency = 0 end
			end
		end
		...
		PageButton.MouseButton1Click:Connect(function()
			...
			for i, v in pairs(MainPage:GetChildren()) do
				if not (v:IsA('UIPageLayout')) and not (v:IsA('UICorner')) then v.Visible = false end
			end
			...
			for i, v in next, ControlList:GetChildren() do
				if not (v:IsA('UIListLayout')) then
					if v.Name == Page_Name .. "_Control" then
						TweenService:Create(v.Frame.Line.PageInLine, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundTransparency = 0 }):Play()
					else
						TweenService:Create(v.Frame.Line.PageInLine, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundTransparency = 1 }):Play()
					end
				end
			end
		end)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			Topsec.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
			...
			LineShadow.BackgroundColor3 = Color3.fromRGB(0,0,0)
			...
			LineShadow.ImageTransparency = 0.7
			...
			if Toggleable then
				...
				VisibilityButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				VisibilitySectionFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
			...
			function sectionFunction:AddToggle(idk, Setting)
				...
				TogFrame1.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				if Desc and Desc ~= "" then
					...
					ToggleDesc.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				local function ChangeStage(val)
					...
					game.TweenService:Create(check, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
						Size = csize, Position = pos, AnchorPoint = apos
					}):Play()
				end
				ChangeStage(Default)
				...
				local controlData = {
					...
					SectionName = Section_Name, TabName = Page_Name, TabButton = PageName
				}
			...
			function sectionFunction:AddButton(Setting, Callback)
				...
				TextColor_1.BackgroundColor3 = Color3.fromRGB(0,0,0)
				...
				if Desc and Desc ~= "" then
					...
					TextDesc.Text = Desc
					TextDesc.TextColor3 = Color3.fromRGB(140, 140, 140)
				...
				ClickArea_1.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
				...
				Frame_1.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
				...
				Button_1.Text = "Click"
				Button_1.TextColor3 = Color3.fromRGB(220, 220, 220)
				...
				Button_1.MouseButton1Down:Connect(function()
					...
					local rippleTween = TweenService:Create(ripple, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
						Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Position = UDim2.new(0.5, 0, 0.5, 0)
					})
					rippleTween:Play()
			...
			function sectionFunction:AddLabel(text)
				...
				LabelBG.BackgroundColor3 = Color3.fromRGB(5,5,5)
				LabelBG.BackgroundTransparency = 0.1
				...
				TextColor.BackgroundColor3 = Color3.fromRGB(0,0,0)
				...
				TextColor.TextColor3 = Color3.fromRGB(220, 220, 210)
			...
			function sectionFunction:AddSeperator(text)
				...
				SeparatorFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				if text and text ~= "" then
					...
					SeparatorLabel.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
			...
			function sectionFunction:AddSlider(Setting)
				...
				SliderButton.Text = ""
				SliderButton.TextColor3 = Color3.fromRGB(220, 220, 220)
				...
				Sliderbox_2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				if DefaultValue then
					if DefaultValue <= minValue then DefaultValue = minValue
					elseif DefaultValue >= maxValue then DefaultValue = maxValue end
					Sliderbox_2.Text = tostring(DefaultValue)
					Bar.Size = UDim2.new(1 - ((maxValue - DefaultValue) / (maxValue - minValue)), 0, 0, 6)
					if Callback then Callback(tonumber(DefaultValue)) end
				end
				...
				RunService.RenderStepped:Connect(function()
					if holdStarted > 0 and (tick() - holdStarted >= holdTime) and not dragging then dragging = true end
				...
				local function GetSliderValue(Value)
					if tonumber(Value) <= minValue then
						Bar.Size = UDim2.new(0, 0, 0, 6); callBackAndSetText(minValue)
					elseif tonumber(Value) >= maxValue then
						Bar.Size = UDim2.new(0, SizeChia, 0, 6); callBackAndSetText(maxValue)
					else
			...
			function sectionFunction:AddInput(idk, Setting)
				...
				Boxtitle.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				Boxxx.Parent = BoxCor
				Boxxx.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				if Number_Only then
					Boxxx:GetPropertyChangedSignal("Text"):Connect(function()
						if not tonumber(Boxxx.Text) then Boxxx.PlaceholderText = Placeholder; Boxxx.Text = '' end
					end)
				end
				Boxxx.FocusLost:Connect(function()
					...
					if Boxxx.Text ~= '' then Callback(Boxxx.Text) end
				end)

				local textbox_function = {}
				if Default then Boxxx.Text = Default end
				function textbox_function.SetValue(Value) Boxxx.Text = Value; Callback(Value) end

				local controlData = {
					Name = TitleText, Section = Section, Element = BoxFrame,
					SectionName = Section_Name, TabName = Page_Name, TabButton = PageName
				}
				table.insert(getgenv().AllControls, controlData)
			...
			function sectionFunction:AddKeyBind(Setting, Callback)
				...
				BindButtonTitle.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				Bindkey.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				Bindkey.MouseButton1Click:Connect(function()
					...
					Connection = uis.InputBegan:Connect(function(input)
						if Picking then
							...
							if Key then
								Picking = false; CurrentKey = Key; Bindkey.Text = Key
				...
				uis.InputBegan:Connect(function(input, gpe)
					...
					elseif input.UserInputType == Enum.UserInputType.MouseButton2 then pressedKey = "MouseRight"
					end
					if pressedKey == CurrentKey then
						if CurrentMode == "Toggle" then
							ToggleState = not ToggleState; pcall(Callback, ToggleState)
						elseif CurrentMode == "Hold" then
							HoldActive = true; pcall(Callback, true)
						end
					end
				end)
				...
				uis.InputEnded:Connect(function(input)
					...
					elseif input.UserInputType == Enum.UserInputType.MouseButton2 then releasedKey = "MouseRight"
					end
				...
				function keybindFunction:SetMode(mode)
					if mode == "Hold" or mode == "Toggle" then
						CurrentMode = mode; ToggleState = false; HoldActive = false
					end
				end
				function keybindFunction:GetMode() return CurrentMode end
				function keybindFunction:GetState()
					if CurrentMode == "Toggle" then return ToggleState
					elseif CurrentMode == "Hold" then return HoldActive end
					return false
				end
			...
			function sectionFunction:AddDropdown(idk, Setting)
				...
				local Callback = Setting.Callback
				local sortPairs = Setting.SortPairs or pairs
				...
				if Search then Dropdowntitle = Instance.new("TextBox")
				else Dropdowntitle = Instance.new("TextLabel") end
				...
				Sel.Value = ""
				if Default and List and table.find(List, Default) then Sel.Value = Default end
				...
				if Default and Callback then task.spawn(function() pcall(Callback, Default) end) end
				...
				Dropdownlisttt.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				DropdownScroll.Name = "DropdownScroll"
				DropdownScroll.Parent = Dropdownlisttt
				DropdownScroll.Active = true
				DropdownScroll.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
				...
				local function edit()
					for i in pairs(found) do found[i] = nil end
				...
				local function SearchDropdown()
					for i, v in pairs(searchtable) do
						if string.find(v, Dropdowntitle.Text) then table.insert(found, v) end
					end
					for a, b in pairs(ScrollContainer:GetChildren()) do
						for c, d in pairs(found) do
							if d == b.Name then b.Visible = true end
						end
					end
				end
				...
				if Selected then
					...
					if Default and Callback then task.spawn(function() Callback(Default, true) end) end
				else
					ListNew = List
				end

				local function refreshlist(SortPairs)
					local usePairs = SortPairs or pairs
					clear_object_in_list()
					searchtable = {}
					for i, v in usePairs(ListNew) do
						if Selected then table.insert(searchtable, string.lower(i))
						else table.insert(searchtable, string.lower(tostring(v))) end
					end
					...
					if Selected then
						for _, i in ipairs(OrderedList) do
							...
							SampleItem.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
							...
							SampleItemBG.BackgroundColor3 = v and getgenv().UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(100, 100, 100)
							...
							SampleItemTitle.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
							...
							SampleItemTitle.TextColor3 = Color3.fromRGB(220, 220, 220)
							...
							SampleItemButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
							...
							SampleItemButton.MouseEnter:Connect(function()
								...
								TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundColor3 = Color3.fromRGB(80,80,80), BackgroundTransparency = 0.6 }):Play()
							end)
							SampleItemButton.MouseLeave:Connect(function()
								if v then return end
								TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundTransparency = 1 }):Play()
							end)
							SampleItemButton.MouseButton1Click:Connect(function()
								v = not v
								TweenService:Create(SampleItemCheck, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { ImageTransparency = v and 0 or 1 }):Play()
								TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
									BackgroundColor3 = v and getgenv().UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(100, 100, 100),
									BackgroundTransparency = v and .5 or 1
								}):Play()
								if Callback then Callback(i, v); ListNew[i] = v end
								if Search then Dropdowntitle.PlaceholderText = Title .. ': '
								else Dropdowntitle.Text = Title .. ': ' end
							end)
						end
					else
						for i, v in usePairs(ListNew) do
							if typeof(v) == "string" then
								...
								SampleItemBG.Parent = SampleItem
								SampleItemBG.AnchorPoint = Vector2.new(0.5, 0.5)
								SampleItemBG.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
								...
								SampleItemButton.MouseEnter:Connect(function()
									...
									TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundColor3 = Color3.fromRGB(80,80,80), BackgroundTransparency = .6 }):Play()
								end)
								SampleItemButton.MouseLeave:Connect(function()
									if Sel.Value == v then return end
									TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), { BackgroundTransparency = 1 }):Play()
								...
								SampleItemButton.MouseButton1Click:Connect(function()
									if Search then Dropdowntitle.PlaceholderText = Title .. ': ' .. v; Sel.Value = v
									else Dropdowntitle.Text = Title .. ': ' .. v; Sel.Value = v end
									TweenService:Create(SampleItemBG, TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
										BackgroundColor3 = getgenv().UIColor["Dropdown Selected Color"],
										BackgroundTransparency = .5
									}):Play()
									if Callback then Callback(v) end
									if Search then Dropdowntitle.Text = "" end
									refreshlist()
								end)
								if Sel.Value == v then
									SampleItemBG.BackgroundTransparency = .5
									SampleItemBG.BackgroundColor3 = getgenv().UIColor["Dropdown Selected Color"]
				...
				if typeof(Default) ~= 'table' then
					if Search then Dropdowntitle.PlaceholderText = Title .. ': ' .. tostring(Default or "")
					else Dropdowntitle.Text = Title .. ': ' .. tostring(Default or "") end
				...
				function dropdownFunction:ClearText(v)
					if Search then Dropdowntitle.PlaceholderText = Title .. ': ' .. (v or "")
					else Dropdowntitle.Text = Title .. ': ' .. (v or "") end
				end
				function dropdownFunction:GetNewList(List)
					...
					TweenService:Create(DropdownFrame, TweenInfo.new(getgenv().UIColor["Tween Animation 2 Speed"]), { Size = UDim2.new(1,0,0,25) }):Play()
				...
				function dropdownFunction:GetValue()
					if not Selected then return Sel.Value
					else
						local result = {}
						for key, val in pairs(ListNew) do
							if val == true then table.insert(result, key) end
						end
						return result
					end
				end
